...
import { SelectableAddresses } from '@/components/Modal/SelectableAddresses';
...
export default function SessionProposalModal() {
  ...
  const addresses = useMemo(() => Array.from(selectedAddresses), [selectedAddresses]);
  // console.log(`SessionProposalModal addresses: ${addresses}`);
  ...
  const supportedNamespaces = useMemo(() => {
    ...
    const accounts = eip155Chains.flatMap(chain =>
      addresses.map(address => `${chain}:${address}`)
    );
    // console.log(`SessionProposalModal accounts: ${accounts}`);
    ...
    // console.log(`SessionProposalModal namespace: ${namespace}`);
    return namespace;
  }, [addresses]);
  ...
  const handleToggleAddress = useCallback((address: string) => {
  ...
  return (
    <RequestModal
      ...
      disableApprove={selectedAddresses.size === 0}>
